...
class OllamaClient:
    ...
    def chat(self, messages: List[Dict[str, str]], model: Optional[str] = None, keep_alive: Optional[str] = None, options: Optional[Dict] = None, get_ui_string=None) -> str:
        ...
        try:
            ...
            def tr(key, default):
                return get_ui_string(key, default) if get_ui_string else default
            return tr("msg_ollama_timeout", f"⌛ O Tutor IA demorou muito para gerar a resposta (mais de {self.timeout} segundos) e a conexão foi encerrada. Isso é normal se o seu computador estiver sobrecarregado ou usando apenas a CPU.\n\nPor favor, **aguarde uns instantes** e envie sua pergunta novamente!")
    ...
    def preload_model(self, model: Optional[str] = None) -> bool:
        """
        Pré-carrega o modelo na VRAM/RAM assim que o aplicativo abre,
        mantendo-o residente (keep_alive: -1) enquanto o Pyeduc estiver aberto.
        """
