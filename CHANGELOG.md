...
### Corrigido
- **Manifesto Linux Flatpak ([flatpak/org.pyeduc.App.yml](flatpak/org.pyeduc.App.yml)):** Adicionada a instrução `sources:` (`type: dir`, `path: ..`) no módulo `pyeduc`, garantindo a cópia dos diretórios `build/linux/` e `content/` para a sandbox do `flatpak-builder` durante a compilação.
