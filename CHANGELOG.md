...
- **Prevenção de Instâncias Duplicadas e Erros de Login ([src/main.py](src/main.py) & [src/gui.py](src/gui.py)):** Substituído `ft.run` por `ft.app(target=main_app)` no ponto de entrada principal, corrigindo a abertura de janelas duplas e avisos do Flutter engine. Adicionada resolução defensiva de IDs e índices em `load_lesson(target)` no `src/gui.py`, prevenindo exceções `IndexError: list index out of range` ao carregar lições do banco de dados.
