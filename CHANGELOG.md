...
### Corrigido
- **Manifesto Linux Flatpak ([flatpak/org.pyeduc.App.yml](flatpak/org.pyeduc.App.yml)):** Efetuado o upgrade de runtime para Freedesktop `24.08`, resolvendo o aviso de fim de vida (EOL) e corrigindo a falha de símbolo indefinido `g_once_init_enter_pointer` (GLib 2.80+). Incluídos o bloco `sources:` e o arquivo de metadados AppStream (`/app/share/metainfo/org.pyeduc.App.metainfo.xml`), corrigindo o nome exibido para `Pyeduc` no `flatpak list`.
