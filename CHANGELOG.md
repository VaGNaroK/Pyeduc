...
### Corrigido
- **Manifesto Linux Flatpak e Pacote DEB ([flatpak/org.pyeduc.App.yml](flatpak/org.pyeduc.App.yml) & [scripts/build_deb.sh](scripts/build_deb.sh)):** Efetuado o upgrade de runtime para Freedesktop `24.08`, resolvendo o aviso de fim de vida (EOL) e a falha `g_once_init_enter_pointer`. Adicionadas as rotas de cópia de assets para `/app/opt/pyeduc/app/assets/content/icons/pyeduc.svg` e configurado `assets_dir = "."` no [pyproject.toml](pyproject.toml), corrigindo a exceção `PathNotFoundException`. Incluído o arquivo AppStream (`org.pyeduc.App.metainfo.xml`), corrigindo o nome para `Pyeduc` no `flatpak list`.
- **Prevenção de Instâncias Duplicadas e Erros de Login ([src/main.py](src/main.py), [src/content_manager.py](src/content_manager.py), [src/progress_manager.py](src/progress_manager.py) & [src/gui.py](src/gui.py)):** Removido `assets_dir=project_root` em `src/main.py`, corrigindo o disparo de janelas secundárias pelo Flet no Serious Python. Adicionada resolução dinâmica de caminhos absolutos para `content/lessons.json` em `ContentManager` e salvamento da base SQLite em `~/.pyeduc/pyeduc.db` em `ProgressManager`, prevenindo exceções `IndexError` no login em ambientes sandbox (Flatpak/DEB). Mapeamento defensivo implementado em `load_lesson(target)` no `src/gui.py`.
