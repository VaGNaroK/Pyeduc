...
- **Testes Unitários da Camada Core (Backend)**: Escritos e aplicados os scripts de testes 100% validados para: Auto-Grader/Validação Fuzzy (`test_main_window_grader.py`), indexação de Busca da Inteligência Artificial RAG (`test_rag_module.py`), Comunicação IPC (`test_communication.py`) e Estado Global (`test_app_state.py`) via injeção de Mocks robustos.
- **Diretriz de TDD no AGENTS.md**: Adicionada a "Regra 21: Cobertura Obrigatória de Testes Unitários" em `.agents/AGENTS.md` forçando futuros modelos a verificarem dependências antes e depois de quaisquer alterações estruturais.
...
- **Manual do Usuário Ilustrado (`MANUAL.md`)**: Criado o documento raiz `MANUAL.md` servindo como guia prático de inicialização da plataforma, focado nas funcionalidades essenciais (Navegação, IDE Console e Uso do Tutor IA Socrático). As imagens demonstrativas estão abrigadas no sub-diretório estrutural `docs/images/`.
- **Botão de Logout (Sair) na TopBar**: Implementado um botão de saída segura que desvincula a conta ativa, oculta de forma unificada as views restritas da plataforma, reseta as permissões de Admin e exibe suavemente a tela de Login (Welcome Container). As strings relativas a essa ação foram incluídas em todos os dicionários de idioma.
...
- **Vazamento de Estado de Lição e Bugs do Footer (Logout)**: Ao deslogar e trocar a língua no Login, a GUI tentava erroneamente renderizar componentes da conta anterior (que estava salva no `current_lesson_idx`), causando colisão entre o Console Python e a Tela de Welcome. O `app_state.py` e o `update_footer` agora incluem proteções rigorosas contra indexação com `NoneType`, e o índice da lição é propriamente anulado ao sair da conta.
...
- **Crash Flet Engine (kInvalidArguments) no Login**: Removida a renderização dupla acidental durante a transição da tela de login. O método `update_ui_strings` agora aceita a flag `skip_lesson_select=True`, evitando que a engine C++ do Flutter tente remover/atualizar visualizações não estabilizadas em blocos síncronos pesados.
- **Out of Bounds (NoneType) na Seleção de Aulas**: Corrigido o erro crítico que quebrava o app em aulas com ID superior a 1000. O sistema lia o ID persistente (ex: `1011`) do SQLite e o injetava diretamente como *índice de array* (0 a 24). Criada a função de mapeamento `get_lesson_index_by_id` no `AppState` para traduzir com segurança a posição real na lista.
- **Hardcode de Tradução UI (`lesson_view.py`)**: Alteradas as legendas "Exemplo:" e "Exercício(s):" para extraírem do sistema unificado de dicionário `get_ui_string`, acompanhando corretamente quando o usuário troca para o inglês.
